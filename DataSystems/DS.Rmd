...
There are two phases of data analysis [@good1983data]: exploratory data analysis (EDA) and confirmatory data analysis (CDA). Table \@ref(tab:edacda) summarizes some differences between EDA and CDA. EDA is usually applied to observational data with the goal of looking for patterns and formulating hypotheses. In contrast, CDA 
...
\end{matrix}
$$
Table: (\#tab:edacda) Comparison of exploratory data analysis and confirmatory data analysis.
...
\]
where $Y$ is a target variable and $X_1$, $X_2$, $\ldots$, $X_p$ are explanatory variables.  Other terms are also used to mean a target variable. Table \@ref(tab:lmnames) gives a list of common names for different types of variables [@frees2009]. When the target variable is a categorical variable, supervised learning methods are called classification methods. When the target variable is continuous, supervised learning methods are called regression methods.
...
\end{matrix}
$$
Table: (\#tab:lmnames) Common names of different variables.
...
\end{matrix}
$$
Table: (\#tab:mlalg) Types of machine learning algorithms.
...
\end{matrix}
$$
Table: (\#tab:desstat) Some commonly used descriptive statistics.

Table \@ref(tab:desstat) lists some commonly used descriptive statistics. In R, we can use the function $\texttt{summary}$ to calculate some of the descriptive statistics. For numeric data, we can visualize the descriptive statistics using a boxplot.
...
\end{matrix}
$$
Table: (\#tab:rfun) Some R functions for data analysis.

Table \@ref(tab:rfun) lists a few R functions for different data analysis tasks. Readers can read the R documentation for examples of using these functions. There are also other R functions from other packages to do similar things. However, the functions listed in this table provide good start points for readers to conduct data analysis in R. For analyzing large datasets in R in an efficient way, readers are referred to [@daroczi2015]. 
